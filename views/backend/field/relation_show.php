        ...
        <tbody>
            <?php foreach ( $view_values['rows'] as $id => $row ): ?>
                <tr>
                    ...
                    <?php endforeach; ?>
                    <td>                    
                        <a class="label label-info" href="<?php echo URL::site( 'admin/' . Inflector::singular($field_name) .'/update/' . $row->obj ); ?>"><i class="icon-cog icon-white"></i> <?php echo __('Edit'); ?></a>
                        <a class="label label-important" href="javascript: (confirm('<?php echo __('Do you have delete object?'); ?>')) ? window.location='<?php echo URL::site( 'admin/' . Inflector::singular($field_name) .'/delete/' . $row->obj ); ?>' : 0 "><i class="icon-trash icon-white"></i> <?php echo __('Delete'); ?></a>
                    </td>
                </tr>
            <?php endforeach; ?>
        </tbody>
